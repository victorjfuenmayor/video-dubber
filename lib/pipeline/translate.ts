...
async function translateSingle(seg: Segment): Promise<string> {
  const targetChars = Math.round(seg.targetDuration * CHARS_PER_SECOND);
  const message = await client.messages.create({
    model: 'claude-sonnet-4-6',
    max_tokens: 512,
    ...
    messages: [{
      role: 'user',
      content: `Translate to Latin American Spanish. Time budget: ${seg.targetDuration.toFixed(1)}s (~${targetChars} chars). Return only the translated text.\n\n${seg.originalText}`,
    }],
  });
  const content = message.content[0];
  if (content.type !== 'text') throw new Error('Unexpected Claude response type');
  ...
  for (let i = 0; i < segments.length; i += BATCH_SIZE) {
    ...
    for (const seg of batch) {
      translateInput[seg.id] = {
        ...
        targetChars: Math.round(seg.targetDuration * CHARS_PER_SECOND),
      };
    }
    ...
    for (const seg of batch) {
      if (!translations[seg.id]) translations[seg.id] = await translateSingle(seg);
    }

    // Pass 2: review timing + Latin American authenticity
