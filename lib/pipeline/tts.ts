...
// Map written forms to how they should be spoken aloud.
  ...
  const res = await fetch(
    `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`,
    {
      ...
      body: JSON.stringify({
        ...
        // so English technical terms embedded in Spanish text are pronounced in English.
        voice_settings: {
          stability: 0.5,
...
export async function generateTTS(segments: Segment[], jobDir: string, voiceId: string): Promise<Segment[]> {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    await generateSegmentAudio(seg.translatedText, audioFile, voiceId);
